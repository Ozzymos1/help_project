...
При работе в команде следует договориться об общих правилах оформления сообщений к коммитам. Следование им позволит взаимодейстовавть более эффективно и упростить понимание кто, куда и зачем внёс в проект изменения.
...
а для английских сообщений следуте использовать повелительное наклонение, т.е. императивы (use, fix и т.д.).
...
Если же требуется откатить коммит, то это делается коммандой **git reset --hard "commit hash"** ("суровый сброс").
После выполнения этой команды состояние репозитория вернётся к коммиту, указанному в ней. 
Т.е. он станет последним (HEAD) и все измениея, которые были после него будут удалены из репозитория.
...
Без парамтров эта команда сравнит последнюю закоммиченную версию файла с его текущей (изменённой) версией. 
...
GIT ценен, прежде всего, тем, что позволяет обеспеспечить взаимодействие команды.
...
На GitHub существует возможность создания копии какого-либо проект и её дальнешее независимое от основного проект существование (в виде отдельного проекта).
Эта возможность называется Fork (ответвление)
Fork позволяте не только развивать собственный проект, но и сделать некие изменения (каторые кажутся полезными) и далее предложить их для внесения в основной проект.
...
По умолчанию ответвтление проекта будет названо также как и основной репозиторий. Далее это имя можно будет изменить.
...
и не мешал другим пока его чать работы не будет сделано.
...
Название веткиможет состоять из букв, цифр, а также включать любой из четырёх символов: **., -, _, /**.
Названия веток должны быть информативными для других членов команды (и для себя тоже).
Как вариант, возможно именовть ветки по шаблону **"указатель ветки"/"имя ветки"**. 
...
Если требуется создать ветку и сразу начать с ней работать это делается предыдущей командой с ключём **-b** (*git checkout -b "название_ветки"*).
...
Вместо флага *-D* можно использововать более безопасный вариант - флаг *-d*. Он удалит ветку только если она была полностью объединена с другой. 
...
**Общие рекомендации по разрешению конфликтов** (файлы, в которых произшёл конфлит GIT подсвечивает, если он не смог их объединить):
...
Чтобы отправить локальную ветку на удалённый репозитарий сначала нужно её создать там (на вкладке *Repositories* нужно нажать кнопку *New*, далее ввести его название, сделать приватным или публичным и далее нажать на кнопку *Create repository*). 
...
Для того, чтообы изменения были слиты с главной веткой необходимо нажать кнопку **Merge pull request**. 
...
Перед приминением этой команды сначала надо перейти в терминале в нужную ветку локального репозитория. 
