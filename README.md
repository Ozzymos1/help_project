...
Для решения этой проблемы «разошедшихся» веток в GIT имеется операция **rebase** (перебазирование). 
...
Советы автору пул-реквеста (подготовить свой пул-реквест так, чтобы его было легко понять и принять):
